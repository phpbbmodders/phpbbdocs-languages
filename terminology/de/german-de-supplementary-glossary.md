# German (de) supplementary glossary

The same genuinely-unconfirmed terms as
[`german-de-x-sie-supplementary-glossary.md`](../de-x-sie/german-de-x-sie-supplementary-glossary.md),
carried over with Du-form wording. See
[`todo-german-de-terminology-audit.md`](todo-german-de-terminology-audit.md)
for why this carries over directly instead of being re-derived from
scratch: phpBB's real UI button/field labels don't change between the
Sie and Du registers, so a term with no matching language-pack key in
one register has no matching key in the other either.

**How to use this file:** for each row, fill in "Agreed German term"
with the wording the team settles on, "Confirmed by" with a name, and
"Date" (YYYY-MM-DD). A row filled in here should also be applied to
the matching row in the `de_x_sie` glossary and vice versa, since both
registers describe the same underlying concept.

| English source | Current German (Du) | Location | Why unconfirmed | Agreed German term | Confirmed by | Date |
|---|---|---|---|---|---|---|
...
Same two issues as the `de_x_sie` glossary, since they're properties
of the real phpBB language-pack keys themselves and don't change
between registers.

Both were reported to phpBB dev team lead Marc, who confirmed both are
genuine documentation bugs (09/2026). phpBB 4.0 work has priority right
now, so a docs fix isn't expected soon — these remain open,
upstream-only issues; nothing to change in this translation.

- **Minimale Vorschaubild-Dateigröße** (admin_guide.xml:1067,
  ...
  `'Minimum thumbnail file size'`, German (both registers):
  `'Minimale Vorschaubild-Dateigröße'` — a *minimum* filesize
  threshold, not a maximum. Marc confirmed: it's a minimum
...
- **Veraltete Vorlagen neu kompilieren** (admin_guide.xml:598,
  ...
  `'Recompile stale style components'`, German (both registers):
  `'Rekompilieren veralteter Style-Komponenten'` — about style
  components, not templates. Marc confirmed: this is a leftover from
  phpBB 3.0 — 3.1 and later only ever had "Recompile stale style
  components."
